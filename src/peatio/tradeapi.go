...
/*
	http://www.Peatio.com/help/index.php?a=api_help
*/
type PeatioTrade struct {
	access_key string
	secret_key string
}

func NewPeatioTrade(access_key, secret_key string) *PeatioTrade {
	w := new(PeatioTrade)
...
func (w *PeatioTrade) httpRequest(pParams map[string]string) (string, error) {
	v := url.Values{}
	...
	logger.Tracef("HTTP req begin PeatioTrade")
	resp, err := c.Do(req)
	logger.Tracef("HTTP req end PeatioTrade")
	if err != nil {
		logger.Fatal(err)
		return "", err
	}
...
func (w *PeatioTrade) check_json_result(body string) (errorMsg ErrorMsg, ret bool) {
...
func (w *PeatioTrade) GetAccount() (account_info Account_info, ret bool) {
...
func (w *PeatioTrade) Get_orders() (ret bool, m []HBOrderItem) {
	pParams := make(map[string]string)
...
func (w *PeatioTrade) Cancel_order(id string) bool {
	pParams := make(map[string]string)
...
func (w *PeatioTrade) BuyBTC(price, amount string) string {
	buyId := w.doTrade("buy", price, amount)
	return (fmt.Sprintf("%d", buyId))
}

func (w *PeatioTrade) SellBTC(price, amount string) string {
