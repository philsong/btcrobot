...
func (this *Dao) Insert() (sql.Result, error) {
	...
	logger.Traceln("Insert sql:", strSql)
	err := this.Open()
	if err != nil {
		return nil, err
	}
...
func (this *Dao) Update() error {
	...
	logger.Traceln("Update sql:", strSql)
	err := this.Open()
	if err != nil {
		return err
	}
...
func (this *Dao) Delete() error {
	...
	logger.Traceln("Delete sql:", strSql)
	err := this.Open()
	if err != nil {
		return err
	}
...
func (this *Dao) Increment(field string, num int) error {
	...
	logger.Traceln("Increment sql:", strSql)
	err := this.Open()
	if err != nil {
		return err
	}
...
func (this *Dao) Count() (total int, err error) {
	...
	logger.Traceln("Count sql:", strSql)
	err = this.Open()
	if err != nil {
		return
	}
...
func (this *Dao) Find(colFieldMap map[string]interface{}, selectCol ...string) error {
	...
	logger.Traceln("Find sql:", strSql)
	err := this.Open()
	if err != nil {
		return err
	}
...
func (this *Dao) FindAll(selectCol ...string) (*sql.Rows, error) {
	...
	this.selectCols = "`" + strings.Join(selectCol, "`,`") + "`"
	strSql := util.SelectSql(this)
	logger.Traceln("FindAll sql:", strSql)
	err := this.Open()
	if err != nil {
		return nil, err
	}
